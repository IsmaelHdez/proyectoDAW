...
foreign key (nutricionista) references nutricionista(id_nutricionista) on delete cascade);
 ...
 insert into paciente(usuario, pass, nombre, apellido, email,id_nutricionista) values ("joselu", "joselu1234", "Jose Luis", "Pajares Paz", "pajarespaz@gmail.com", 2);
 ...
 delete from citas where nutricionista = (select id_nutricionista from nutricionista where usuario = 'picha');
 select distinct n.usuario from nutricionista n join citas c on n.id_nutricionista = c.nutricionista;
 select distinct p.usuario, p.nombre, p.apellido, p.email ,m.altura , m.peso ,
    m.grasa_corporal , m.musculo from paciente p 
    join medidas_paciente m on p.id_paciente = m.id_paciente where p.id_nutricionista = 3;
